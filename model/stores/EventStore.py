...
class EventStore(Store, ABC):
    ...
    def store(self, datalake: Datalake, event):
        self.writer.write_metadata(self._metadata_file_name(datalake, event), self.metadata_serializer.serialize(event.metadata()))
    ...
    def _data_filename(self, datalake, event):
        return datalake.directory + "/" + str(event.metadata().id) + ".jpg"

    def _metadata_file_name(self, datalake, event):
        return datalake.directory + "/metadata/" + str(event.metadata().id) + ".metadata"
